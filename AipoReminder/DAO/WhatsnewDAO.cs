...
namespace AipoReminder.DAO
{
    class WhatsnewDAO
    {
        ...
        public void GetWhatsnewMsgboardInfo(System.Data.DataSet data)
        {
            ...
            sqlbldr.AppendLine("        from eip_t_whatsnew t1");
            sqlbldr.AppendLine("        where 1 = 1");
            sqlbldr.AppendLine("        and t1.parent_id = -1");
            sqlbldr.AppendLine("        and t1.portlet_type = " + DBConstants.WHATS_NEW_TYPE_MSGBOARD_TOPIC);
            sqlbldr.AppendLine("        or (");
            sqlbldr.AppendLine("            t1.parent_id = 0");
            sqlbldr.AppendLine("            and t1.portlet_type = " + DBConstants.WHATS_NEW_TYPE_MSGBOARD_TOPIC);
            sqlbldr.AppendLine("        )");
            ...
            sqlbldr.AppendLine("            from eip_t_whatsnew t2 ");
            sqlbldr.AppendLine("            where 1 = 1 ");
            sqlbldr.AppendLine("            and t1.whatsnew_id = t2.parent_id");
            sqlbldr.AppendLine("            and t2.user_id = :user_id");
            sqlbldr.AppendLine("            and t2.portlet_type = " + DBConstants.WHATS_NEW_TYPE_MSGBOARD_TOPIC);
            sqlbldr.AppendLine("        )");
            sqlbldr.AppendLine("    )");
